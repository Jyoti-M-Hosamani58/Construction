...
from Am_app.models import Login ,Service_image, Residential,Building,Struturaldesign,Renovation,Swimming,Freelancing,Joinventure,Landscape,Fabrication,Paintcontract,Retro,Completed_project,Elevation,Dop,Certificate,Listofmachine,Gallery,JobApplication,EPF,ISO,LEI,MSME,ESIC,Oproject,Project,Contract,Contact
...
def upload_completed_project(request):
    if request.method == 'POST':
        ...
        if pdf:
            ...
            dop = Project(pdf=pdf, description=description)
            dop.save()

            return redirect('completed_project')  # Redirect to a success page or another view

    return render(request, 'completed_project.html')


def completed_project_view(request):
    projects = Project.objects.all()  # Fetch all Residential objects
    return render(request, 'completed_project_view.html', {'projects': projects})


def completed_project_view1(request):
    projects = Project.objects.all()  # Fetch all Project objects
...
def upload_ongoing_project(request):
    if request.method == 'POST':
        ...
        if pdf:
            # Save the uploaded file and description
            dop = Oproject(pdf=pdf, description=description)
            dop.save()

            return redirect('ongoing_project')  # Redirect to a success page or another view

    return render(request, 'ongoing_project.html')


def ongoing_project_view(request):
    projects = Oproject.objects.all()  # Fetch all Residential objects
    return render(request, 'ongoing_project_view.html', {'projects': projects})


@csrf_exempt  # Only if you don't want CSRF validation in this case, but it's recommended to add CSRF token in the frontend
def delete_oproject(request, dop_id):
    if request.method == 'DELETE':
        try:
            # Retrieve the certificate object and delete it
            dop = Oproject.objects.get(id=dop_id)
            dop.delete()
            return JsonResponse({'success': True})
        except Oproject.DoesNotExist:
            return JsonResponse({'success': False, 'error': 'Certificate not found.'}, status=404)
    return JsonResponse({'success': False, 'error': 'Invalid request method.'}, status=405)
def ongoing_project_view1(request):
    dop_objects = Oproject.objects.all()  # Retrieve all Oproject records
    return render(request, 'ongoing_project_view1.html', {'dop_objects': dop_objects})
